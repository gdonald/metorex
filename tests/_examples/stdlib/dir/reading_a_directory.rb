...
root = File.join(Dir.tmpdir, "metorex_dir_example")
if Dir.exist?(root)
  Dir.each_child(root) { |name| File.delete(File.join(root, name)) }
  Dir.delete(root)
end
Dir.mkdir(root)
p(Dir.empty?(root))
...
names = []
Dir.foreach(root) { |name| names << name }
p(names.sort)

children = []
Dir.each_child(root) { |name| children << name }
p(children)
